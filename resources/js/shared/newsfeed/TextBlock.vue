    ...
    <div class="name-dropdown">
      ...
      <div class="news-action-dropdown" v-click-outside="closeActionDropdown">
        ...
        <div class="action-dropdown-menu" :class="{'show': collapsed}">
          ...
          <a class="dd-item" v-if="item.userId== item.userId"  v-on:click="showModal = true" @click="removeText(item.id, item.projectId)">
            Delete
          </a>
        </div>
      </div>
    </div>
    <div class="type" :class="getWAVEtype(item.type)">TEXT</div>
    <div class="project-name" :class="getWAVEtype(item.type)">{{item.uri}}</div>
    <div class="project-image mt-3">

    </div>


    <ModalConfirmation v-if="showModal" :deleteId="deleteId" :type="type" :projectId="item.projectId">
