<template>
  <div class="modal fade" id="modal-remove-project" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <input type="hidden" name="profileId" class="profileId" id="profileId">
        <input type="hidden" name="projectId" class="projectId" id="projectId">
...
<script>
  ...
  export default {
    ...
    methods:{
      deleteProfile() {
        var profileId = $("#modal-remove-project").find('#profileId').val();
        var projectId = $("#modal-remove-project").find('#projectId').val();
        axios.delete(apiRoute+ '/user/profiles/'+profileId+'/projects/'+ projectId).then(response => {
          $('#modal-remove-project').modal('hide');
...
<style scoped>
  ...
  .fa.pull-right, .timeline .timeline-post .comment-info .pull-right.action-link.action-remove, .pull-right.action-link.action-remove, .pull-right.popover-icon {
    margin-left: 0.3em;
  }
