...
class MediaPostController extends Controller
{
    /**
     * @OA\Get(
     *     path="user/posts/{postId}/media",
     *     summary="Get a list of media based on postId",
     *     tags={"mediaProject"},
     *     description="Returns all media from the system that the user has access to.",
     *     operationId="findMediaProject",
     *     @OA\Parameter(
     *         name="search",
     *         in="query",
    ...
    public function index(Request $request, $postId)
    {
        ...
        $postmedia =  $post->media()->paginate(10);

        // Search for summary text
        if ($request->has('search')) {
            $postmedia->where('name', $request->input('search'));
        }

        return (new MediaCollection($postmedia))
            ->response()
            ->setStatusCode(200);
    }
    ...
    public function store(Request $request, $postId)
    {
        ...
        $media = Media::create([
            ...
            'created_by_image'  => $request->createdByImage,
        ]);
        ...
        if ($request->hasFile('imageUris')) {
            ...
            Storage::disk('public')->putFile('posts/mediaPostImage/media/'.$media->id, $image);

            Media::where('id', $media->id)->update(['uri' =>  $image->hashName()]);
        }


        $postMedia = $post->media()->where('media.id', $media->id)->first();

        return (new MediaResource($postMedia))
            ->response()
            ->setStatusCode(201);
    }
    ...
    /**
     * @OA\Get(
     *     path="user/posts/{postId}/media/{mediaId}",
     *     summary="Get a single Media based on the  postId and tagId",
     *     tags={"mediaProject"},
    ...
    public function show($postId, $mediaId)
    {
        ...
        $postMedia = $post->media()->where('media.id', $mediaId)->first();
     ...
     * @OA\Delete(
     *     path="user/posts/{postId}/media/{mediaId}",
     *     description="Delete a single Media based on the  postId",
     *     operationId="deleteMedia based on the  postId",
     *     summary="Delete a Media based on the  postId",
     *     tags={"mediaProject"},
     *     @OA\Parameter(
     *         description="postId",
     *         in="path",
    ...
    public function destroy($postId, $mediaId)
    {
        ...
        Mediable::where('media_id', $mediaId)->where('mediable_id', $postId)
